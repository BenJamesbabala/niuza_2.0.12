...
import com.xunlei.library.pulltorefresh.internal.FlipLoadingLayout;
import com.xunlei.library.pulltorefresh.internal.LoadingLayout;
import com.xunlei.library.pulltorefresh.internal.RotateLoadingLayout;
import com.xunlei.library.pulltorefresh.internal.Utils;
import com.xunlei.library.pulltorefresh.internal.ViewCompat;
...
//    }
    ...
    protected final void setHeaderScroll(int value) {
        ...
        if (getMode() != Mode.PULL_FROM_START || this.mIsPressing || value >= 0 || value >= this.mLastValue) {
            switch (getPullToRefreshScrollDirection()) {
                case HORIZONTAL:
                    scrollTo(value, 0);
                    break;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrFooterRefreshViewShow) && !a.getBoolean(R.styleable.PullToRefresh_ptrFooterRefreshViewShow, true)) {
            setFooterViewNotShow();
        }
        if (a.hasValue(R.styleable.PullToRefresh_ptrHeaderRefreshViewShow) && !a.getBoolean(R.styleable.PullToRefresh_ptrHeaderRefreshViewShow, true)) {
    ...
    public void addHeaderPaddingTop(int addTop) {
        if (this.mHeaderLayout != null) {
            this.mHeaderLayout.setPaddingTop(addTop);
        }
    }

    public void addFooterPaddingBottom(int addBottom) {
        if (this.mFooterLayout != null) {
            this.mFooterLayout.setPaddingBottom(addBottom);
        }
    }
