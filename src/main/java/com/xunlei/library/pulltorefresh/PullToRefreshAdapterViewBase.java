...
import com.xunlei.library.pulltorefresh.PullToRefreshBase.OnLastItemVisibleListener;
import com.xunlei.library.pulltorefresh.internal.IndicatorLayout;
...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener {
    ...
    public void setAdapter(ExpandableListAdapter adapter) {
        if (this.mRefreshableView instanceof ExpandableListView) {
            ((ExpandableListView) this.mRefreshableView).setAdapter(adapter);
        }
    }

    public void expandGroup(int index) {
        if (this.mRefreshableView instanceof ExpandableListView) {
            ((ExpandableListView) this.mRefreshableView).expandGroup(index);
        }
    }

    public void collapseGroup(int groupPos) {
        if (this.mRefreshableView instanceof ExpandableListView) {
            ((ExpandableListView) this.mRefreshableView).collapseGroup(groupPos);
        }
    }
    ...
    public void removeEmptyView() {
        if (this.mEmptyView != null) {
            getRefreshableViewWrapper().removeView(this.mEmptyView);
            ((AbsListView) this.mRefreshableView).setEmptyView(null);
    ...
    private boolean isLastItemVisible() {
        ...
        if (lastVisiblePosition >= lastItemPosition - 1) {
            ...
            if (lastVisibleChild != null) {
                return lastVisibleChild.getBottom() + -1 <= ((AbsListView) this.mRefreshableView).getBottom();
            }
        }
        return false;
    }
